...
flights %>% 
   filter(!is.na(Destination), !is.na(DepartureDelay)) %>%
    group_by(Destination) %>% 
    summarize(mean_delay = mean(DepartureDelay, na.rm = TRUE))%>%
...
flights %>%
  ...
  ggplot() + geom_bar(aes(x=mean_dep_delay, y=Carrier), stat = 'identity')
