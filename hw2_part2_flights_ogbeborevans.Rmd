...
flights_actual <- filter(flights, Cancelled == 0)
flights_actual
flights <- flights_actual
...
```{r fix_FlightDate}
flights$timestamp <- as.POSIXct(as.Date(flights$FlightDate, "%m/%d/%y"))
str (flights$timestamp)
flights$FlightDate <- flights$timestamp
flights <- subset (flights, select = -timestamp)
```
...
```{r flights_by_dest}
summarize(group_by(flights, Destination), num_flights = n())
...
summarize(group_by(flights, Destination), mean_delay = mean(DepartureDelay, na.rm = TRUE))%>%
...
group_by(Destination) %>%
  summarize(
    count = n(),
    mean_dep_delay = mean(DepartureDelay, na.rm = TRUE),
    p50_dep_delay = quantile(DepartureDelay, 0.50, na.rm = TRUE),
    p95_dep_delay = quantile(DepartureDelay, 0.95, na.rm = TRUE)
  )
...
```{r q32}
ddelay_carr_dest <- flights %>% 
  filter(!is.na(Carrier), !is.na(Destination)) %>%
  group_by(Carrier, Destination) %>% 
  summarize(median_delay = median(DepartureDelay)) %>% 
  arrange(desc(median_delay))
  
 head(ddelay_carr_dest, 10)
```
...
```{r q32_hackerxtra}
ddelay_carr_dest_hacker <- flights %>% 
  filter(!is.na(Carrier), !is.na(Destination)) %>%
  group_by(Carrier, Destination) %>% 
  summarize(median_delay = median(DepartureDelay),
            mean_delay = mean(DepartureDelay),
            num_flights = n()) %>%
  filter(num_flights >= 25) %>% 
  arrange(desc(median_delay))

  head(ddelay_carr_dest_hacker, 10) %>%
   arrange(desc(mean_delay))
...
flights$DepartureHour <- time.to.hour(flights$DepartureTime)
flights[,c(14,22)]
```
...
```{r departure_period}
flights$period <- cut(flights$DepartureHour,
...
```{r dow}
flights$DayOfWeek <- weekdays(flights$FlightDate)
flights[,c("FlightDate","DayOfWeek")]
```
...
> From the summary stats, DepartureDelays are related to Period. Flights in the early and late hours of the day ([0,5], [19,23]) have higher delay times than other flights.
...
g +
  geom_boxplot(aes(x= period, y=DepartureDelay))
```

```{r}
g +
  geom_violin(aes(x= period, y=DepartureDelay))
```

```{r}
g + 
  geom_point(aes(x=DepartureHour, y=DepartureDelay, colour=period))
...
flights %>%
  group_by(Carrier) %>%
  summarize(mean_dep_delay = mean(DepartureDelay, na.rm = TRUE)) %>% 
  ggplot() + geom_bar(aes(x=Carrier, y=mean_dep_delay), stat = 'identity')
...
flights %>%
  group_by(Carrier) %>%
  summarize(mean_dep_delay = mean(DepartureDelay, na.rm = TRUE)) %>% 
  ggplot() + geom_bar(aes(reorder(Carrier,-mean_dep_delay), y=mean_dep_delay), stat = 'identity')
...
```{r save}
saveRDS(flights, file = "data/flights_ogbeborevans.RData")
```
